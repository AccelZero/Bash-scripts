...
if [[ $replace != "replace" ]] && find /etc/nginx/bx/site_avaliable/dev$project\.conf > /dev/null 2> /dev/null
then
  echo "Конфиг nginx dev${project}.conf уже существует"
  check="false"
fi
...
if [[ $replace != "replace" ]] && find /etc/nginx/bx/site_avaliable/dev$project\-ssl.conf > /dev/null 2> /dev/null
then
  echo "Конфиг nginx dev${project}-ssl.conf уже существует"
  check="false"
fi
...
if [[ $replace != "replace" ]] && find /etc/httpd/bx/conf/dev${project}.conf > /dev/null 2> /dev/null
then
  echo "Конфиг apache dev${project}.conf уже существует"
  check="false"
fi
...
unlink /etc/nginx/bx/site_enabled/dev${project}.conf
unlink /etc/nginx/bx/site_enabled/${project}-ssl.conf
unlink /etc/nginx/bx/site_enabled/dev${project}-ssl.conf
mv -f /etc/nginx/bx/site_avaliable/$project\.conf /etc/nginx/bx/site_avaliable/$project\.conf.back-${current_year} 
mv -f /etc/nginx/bx/site_avaliable/dev$project\.conf /etc/nginx/bx/site_avaliable/dev$project\.conf.back-${current_year} 
mv -f /etc/nginx/bx/site_avaliable/$project\-ssl.conf /etc/nginx/bx/site_avaliable/$project\-ssl.conf.back-${current_year} 
mv -f /etc/nginx/bx/site_avaliable/dev$project\-ssl.conf /etc/nginx/bx/site_avaliable/dev$project\-ssl.conf.back-${current_year} 
mv -f /etc/httpd/bx/conf/${project}.conf /etc/httpd/bx/conf/${project}.conf.back-${current_year} 
mv -f /etc/httpd/bx/conf/dev${project}.conf /etc/httpd/bx/conf/dev${project}.conf.back-${current_year} 
...
chown bitrix:bitrix $route_dev
echo "<?php phpinfo(); ?>" >> $route/index.php
echo "<?php phpinfo(); ?>" >> $route_dev/index.php
touch $route/.htsecure
touch $route_dev/.htsecure
...
EOF
cat <<EOF >> /etc/nginx/bx/site_avaliable/dev$project\.conf
	# Default website
	server {
		...
		set \$proxyserver	"http://127.0.0.1:$port_dev";
		set \$docroot		"$route_dev";

		index index.php;
		root $route_dev;

		# Redirect to ssl if need
		if (-f $route_dev/.htsecure) { rewrite ^(.*)$ https://\$host\$1 permanent; }
...
EOF
cat <<EOF >> /etc/nginx/bx/site_avaliable/dev$project-ssl.conf
	# Default SSL certificate enabled website
	server {
		listen 443 ssl http2;
		server_name dev$project;
		...
		access_log /var/log/nginx/dev$project.access.ssl.log main;
		error_log /var/log/nginx/dev$project.error.ssl.log;
...
ln -s /etc/nginx/bx/site_avaliable/dev$project\.conf /etc/nginx/bx/site_enabled/dev$project\.conf
ln -s /etc/nginx/bx/site_avaliable/$project-ssl.conf /etc/nginx/bx/site_enabled/$project-ssl.conf
ln -s /etc/nginx/bx/site_avaliable/dev$project-ssl.conf /etc/nginx/bx/site_enabled/dev$project-ssl.conf
...
cat <<EOF >> /etc/httpd/bx/conf/dev$project\.conf
Listen 127.0.0.1:$port_dev
<VirtualHost 127.0.0.1:$port_dev>
	ServerAdmin webmaster@localhost
	ServerName dev$project
	DocumentRoot $route_dev
	php_admin_value mbstring.func_overload 0
	...
	<Directory $route_dev/bitrix/cache>
		AllowOverride none
        Require all denied
	</Directory>

	<Directory $route_dev/bitrix/managed_cache>
		AllowOverride none
        Require all denied
	</Directory>

	<Directory $route_dev/bitrix/local_cache>
		AllowOverride none
        Require all denied
	</Directory>

	<Directory $route_dev/bitrix/stack_cache>
		AllowOverride none
        Require all denied
	</Directory>

	<Directory $route_dev/upload>
		AllowOverride none
		AddType text/plain php,php3,php4,php5,php6,phtml,pl,asp,aspx,cgi,dll,exe,ico,shtm,shtml,fcg,fcgi,fpl,asmx,pht
		php_value engine off
	</Directory>

	<Directory $route_dev/upload/support/not_image>
		AllowOverride none
        Require all denied
	</Directory>

	<Directory $route_dev/bitrix/images>
		AllowOverride none
		AddType text/plain php,php3,php4,php5,php6,phtml,pl,asp,aspx,cgi,dll,exe,ico,shtm,shtml,fcg,fcgi,fpl,asmx,pht
		php_value engine off
	</Directory>

	<Directory $route_dev/bitrix/tmp>
		AllowOverride none
		AddType text/plain php,php3,php4,php5,php6,phtml,pl,asp,aspx,cgi,dll,exe,ico,shtm,shtml,fcg,fcgi,fpl,asmx,pht
		php_value engine off
	</Directory>

	ErrorLog logs/dev${project}_error.log
	# Possible values include: debug, info, notice, warn, error, crit, alert, emerg.
	LogLevel warn

	CustomLog logs/dev${project}_access.log combined
...
mysql -u root -e "create database dev$project_mysql DEFAULT CHARACTER SET utf8 COLLATE utf8_unicode_ci;"
mysql -u root -e "grant all privileges on $project_mysql.* to $project_mysql@localhost;"
mysql -u root -e "grant all privileges on dev$project_mysql.* to $project_mysql@localhost;"
echo "БД $project_mysql и dev$project_mysql созданы"
...
cat <<EOF 
-----------------------------------------------------------
Проект ${project}/dev${project} успешно создан
-----------------------------------------------------------
EOF
  ...
  then
   ...
   do
     ...
     then
       ...
       setfacl -R -m u:${user}:rwx $route_dev
       setfacl -R -m d:u:${user}:rwx $route_dev
       echo "Права на директории $route и $route_dev пользователю $user выданы"
       read -r -p "Введите имя пользователя, которому нужно выдать права (введите 'N' для выхода): " user
     fi
   done
  elif [[ $answer = "N" ]]
